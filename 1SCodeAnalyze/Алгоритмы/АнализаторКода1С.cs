...
*/
    ...
    class АнализаторКода1С
    {
        ...
        private void ОбойтиВсеФайлы()
        {
            ...
            Console.WriteLine("Будет проанализировано "+Модули.Count.ToString()+" Модулей");
			...
			foreach (KeyValuePair<String, Модуль> Объект in Модули){
                ...
                if (Объект.Value.ЕстьОшибки)
                {
                    ...
                    {
                        Console.WriteLine("строка "+Объект.Value.ПолучитьНомерСтрокиПоИндексу(T.Смещение) + ": " + T.ОписаниеПроблемы);
                    }
                }
                КоличествоМодулей--;
			}
        ...
        private Boolean РекурсивныйПоискЗапроса(String Текст, Модуль МодульОбъекта, int Index,  СвойстваМетодов СвойствоМетода, String ВызывающийМетод, int Глубина)
        {
            ...
            foreach (Match Вызов in Найдены)
            {
				...
				if(РекурсивныйПоискЗапроса(Найден.Groups[2].Value, МодульОбъекта, Найден.Index, СвойствоМетода, Вызов.Value, Глубина + 1)){ 
					СвойствоМетода.ЕстьЗапрос = true;
					СвойствоМетода.ДобавитьВызов(Вызов.Value);
                    if (Глубина == 0) {
                        String СтекСтрокой = СвойствоМетода.ПолучитьСтекСтрокой();
                        ИнформацияАнализа Анализ = new ИнформацияАнализа(Вызов.Index, СтекСтрокой, СтекСтрокой);
...
/*                ТелоКода Тело = new ТелоКода(Функция.Value, МодульОбъекта, Функция.Index);
                ...
                MatchCollection Вызовы = Тело.НайтиВызовы();
                foreach (Match Вызов in Вызовы)
				{
                    СвойстваМетодов СвойствоМетода = new СвойстваМетодов();
                    var ПоискМетода = new Regex(@"(Процедур|Функци|procedur|functio)[аяne][\s]*?" + ЭкранироватьРег(Вызов.Value) + @"[\s]*?\(([\S\s]*?)(Конец|end)\1[ыиen]", RegexOptions.IgnoreCase | RegexOptions.Multiline);
                    Match Найден = ПоискМетода.Match(МодульОбъекта.Текст);
                    if (!Найден.Success)
                        continue;
                    //натравим на эти процедуры эту же функцию
                    if (РекурсивныйПоискЗапроса(Найден.Groups[2].Value, МодульОбъекта, Найден.Index, СвойствоМетода, Вызов.Value, 0))
                    {
                        СвойствоМетода.ЕстьЗапрос = true;
                        СвойствоМетода.ДобавитьВызов(Вызов.Value);
                        String СтекСтрокой = СвойствоМетода.ПолучитьСтекСтрокой();
